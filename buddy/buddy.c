...
typedef struct {
	...
	int index;				//index for pages 
	char* address;			//address for pages
	int order;				//order for pages
} page_t;
	...
	for(int i = alloc_order; i <= MAX_ORDER; i++){
		if(!list_empty(&free_area[i])){
			...
			if(i == alloc_order){
				...
				int right_ind = temp_left->index + (ORDER_SIZE(alloc_order)/PAGE_SIZE);
				list_add(&(g_pages[right_ind].list), &free_area[alloc_order]);
			}
